...
export function init(SCENE) {
  ...
  // 几何图形
  const sphere = getSphereGeometry(globalConfig.airshipRadius, 8, 8);
  // 材质
...
export function update(CAMERA, SCENE, AIRSHIP, FLY_CONTROLS) {
  ...
  // 飞船位置
  const _x = globalConfig.airshipDistance * direction.x + position.x;
  const _y = globalConfig.airshipDistance * direction.y + position.y;
  const _z = globalConfig.airshipDistance * direction.z + position.z;
  // 碰撞检测
...
function collisionDetection(SCENE, AIRSHIP) {
  ...
  return new Promise((resolve, reject) => {
    ...
    meshList.forEach(obj => {
      ...
      if (_star && distance - (_radius + globalConfig.airshipRadius) < globalConfig.collisionThreshold) {
        return resolve({
          id: _id,
          name: _name
        });
